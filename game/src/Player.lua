...
function Player:update(dt)
    ...
    if self.hasKey and not self.consumingKey then
        Timer.tween(0.15, {
            [self] = {keyx = self.x+self.width/2, keyy = self.y + 8}
        })
    end
end
...
function Player:render()
    Entity.render(self)
    if self.hasKey then
        love.graphics.draw(gTextures['keys-and-locks'], gFrames['keys-and-locks'][self.key],
            math.floor(self.keyx), self.keyy, 0, self.direction == 'right' and 1 or -1, 1, 24, 0)
    end
end
    ...
    for k, object in pairs(self.level.objects) do
        if object:collides(self) then
            ...
            elseif object.consumable then
                if object.onConsume(self, object) then
                    if self.consumingKey then
